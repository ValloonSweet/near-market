...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_transfer(
        ...
        // assert that the user attached exactly 1 yoctoNEAR. This is for security and so that the user will be
    ...
    fn nft_transfer_call(
        ...
        ext_non_fungible_token_receiver::ext(receiver_id.clone())
            ...
            .then(
                // Defaulting GAS weight to 1, not aatached depsoit, and static GAS equal to the GAS for resolve transfer
...
impl NonFungibleTokenResolver for Contract {
    ...
    fn nft_resolve_transfer(
        ...
        if let PromiseResult::Successful(value) = env::promise_result(0) {
            // As per the standard, the nft_on_transfer should return whether we should return the token
            // to it's owner or not
            if let Ok(return_token) = near_sdk::serde_json::from_slice::<bool>(&value) {
                // if we need don't need to return the token, we simply return true meaning everything went fine
                if !return_token {
                    /*
                        since we've already transeferred the token and nft_on_trasfer returned false, we don't
                        have to revert the original transfer and thus we can just return true since nothing went wrong
                    */
